...
# For products sold no storno: just subtract stornos from integer sum
def get_products_sold_no_storno(df, product_cols_int):
    total_products = df[product_cols_int].sum().sum()
    stornos = df[df['storno'] == 1].shape[0]
    return total_products - stornos
...
if sales_file and calls_file:
    ...
    if page == "Dashboard":
        ...
        # 1. AGENT FILTER (for stats and graphs)
        ...
        # Team total for percent of team sales (ALWAYS All Agents in same period)
        team_df_for_period = df_team_period.copy()
        team_df_for_period = make_product_int_cols(team_df_for_period)
        team_product_cols_int = [col + '_int_only' for col in product_cols if col + '_int_only' in team_df_for_period.columns]
        team_total_products = team_df_for_period[team_product_cols_int].sum().sum()
        team_stornos = team_df_for_period[team_df_for_period['storno'] == 1].shape[0]
        team_total_products_no_storno = team_total_products - team_stornos
        ...
        # Calls/Day, Calls >3min/Day, Avg Product/Day (exclude weekends!)
        call_df = df_calls_filtered.copy()
        call_df['date'] = pd.to_datetime(call_df['Start time'], errors='coerce').dt.date
        ...
        df_filtered['day'] = pd.to_datetime(df_filtered['createdate_berlin'], errors='coerce').dt.date
        ...
        # Avg break between calls (only for individuals)
        if selected_agent == "All Agents":
        ...
        if selected_date == "All Time":
            ...
            # Sales Rate
            deals_per_period = df_filtered.groupby('month_period')[['full_name']].nunique()['full_name']
            ...
            # For monthly (daily, weekends excluded)
            df_filtered = exclude_weekends(df_filtered, 'createdate_berlin')
        ...
        # Use only filtered agent+period
