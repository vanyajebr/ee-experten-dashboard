...
def summary_by_period(df, period_col):
    # Group by period, sum products, and count "real" stornos (storno==1 and >0 products)
    product_cols_int = [col + '_int_only' for col in product_cols if col + '_int_only' in df.columns]
    grouped = df.groupby(period_col)
    products_sold = grouped[product_cols_int].sum().sum(axis=1)
    # Only count storno rows that actually cancel a product (not phantom stornos)
    stornos = grouped.apply(lambda x: ((x['storno'] == 1) & (x[product_cols_int].sum(axis=1) > 0)).sum())
    products_sold_no_storno = products_sold - stornos
    # For deals: count unique full_names per period
    full_names = grouped.apply(lambda x: (x['kunde_vorname'].astype(str).str.strip() + " " + x['kunde_nachname'].astype(str).str.strip()).nunique())
    return pd.DataFrame({
        'products_sold': products_sold,
        'stornos': stornos,
        'products_sold_no_storno': products_sold_no_storno,
        'deals': full_names
    })
...
if sales_file and calls_file:
    ...
    if page == "Dashboard":
        ...
        # -- DASHBOARD METRICS (for the selected agent and period)
        products_sold = df_filtered[product_cols_int].sum().sum()
        # Only count storno rows that have at least one product in the row
        stornos = df_filtered[(df_filtered['storno'] == 1) & (df_filtered[product_cols_int].sum(axis=1) > 0)].shape[0]
        products_sold_no_storno = products_sold - stornos
        ...
        storno_rate = (stornos / products_sold * 100) if products_sold else 0

        # For % of team sales
        df_team_products_sold = df_team_period[product_cols_int].sum().sum()
        df_team_stornos = df_team_period[(df_team_period['storno'] == 1) & (df_team_period[product_cols_int].sum(axis=1) > 0)].shape[0]
        df_team_products_sold_no_storno = df_team_products_sold - df_team_stornos
        percent_of_team_sales = (
            "100.0%" if selected_agent == "All Agents" else
            f"{(products_sold_no_storno / df_team_products_sold_no_storno * 100):.1f}%" if df_team_products_sold_no_storno else "—"
        )

        # -- Calls, unique numbers, calls >3min, avg call duration >3min
        ...
        # -- Daily stats (exclude weekends for averages)
        ...
        # -- Avg product sold/day (no storno, business logic: products - stornos, daily)
        ...
        # Only count stornos with products in daily subtraction
        daily_stornos = df_filtered.groupby('day').apply(lambda x: ((x['storno'] == 1) & (x[product_cols_int].sum(axis=1) > 0)).sum())
        sales_no_storno_per_day = sales_per_day.sum(axis=1) - daily_stornos
        avg_product_sold_per_day = f"{sales_no_storno_per_day.mean():.2f}" if hasattr(sales_no_storno_per_day, 'mean') else "—"

        # -- Avg sale time of day
        ...
        col2.metric("Total Products Sold", products_sold)
        col3.metric("Total Stornos", stornos)
        col4.metric("Total Products Sold (No Storno)", products_sold_no_storno)
        ...
        # --- GRAPHS: Use summary_by_period everywhere ---
        if selected_date == "All Time":
            df_filtered['month_period'] = pd.to_datetime(df_filtered['createdate_berlin'], errors='coerce').dt.to_period('M')
            summary = summary_by_period(df_filtered, 'month_period')
            manual_order = ["2024-12", "2025-01", "2025-02", "2025-03", "2025-04"]
            for col in summary.columns:
                summary[col] = summary[col].reindex(manual_order, fill_value=0)
                summary[col].index = pd.CategoricalIndex(summary.index.astype(str), categories=manual_order, ordered=True)
            # Products Sold No Storno
            st.subheader("Products Sold (No Storno) Over Time (Monthly)")
            st.line_chart(summary['products_sold_no_storno'])
            # Sales Rate
            # Calculate unique_called_numbers and deals for graph periods
            calls_summary = df_calls_filtered.copy()
            calls_summary['month_period'] = pd.to_datetime(calls_summary['Start time'], errors='coerce').dt.to_period('M')
            calls_per_period = calls_summary.groupby('month_period')['Called number'].nunique().reindex(manual_order, fill_value=0)
            deals_per_period = summary['deals']
            sales_rate_per_period = (deals_per_period / calls_per_period * 100).fillna(0)
            st.subheader("Sales Rate Over Time (Monthly)")
            st.line_chart(sales_rate_per_period)
            # Storno Rate
            storno_rate_per_period = (summary['stornos'] / summary['products_sold'] * 100).replace([np.inf, -np.inf], 0).fillna(0)
            ...
            st.line_chart(calls_per_period)
        else:
            # Daily breakdowns, exclude weekends
            df_filtered = filter_period(df_sales, 'createdate_berlin', selected_date)
            df_calls_filtered = filter_period(df_calls, 'Start time', selected_date)
            df_filtered['createdate_berlin'] = pd.to_datetime(df_filtered['createdate_berlin'], errors='coerce')
            df_calls_filtered['Start time'] = pd.to_datetime(df_calls_filtered['Start time'], errors='coerce')
            df_filtered = make_product_int_cols(df_filtered)
            df_filtered = exclude_weekends(df_filtered, 'createdate_berlin')
            df_calls_filtered = exclude_weekends(df_calls_filtered, 'Start time')
            df_filtered['day'] = df_filtered['createdate_berlin'].dt.day
            summary = summary_by_period(df_filtered, 'day')
            # Products Sold No Storno
            st.subheader(f"Products Sold (No Storno) Over Time (Daily, excl. weekends) - {selected_date}")
            st.line_chart(summary['products_sold_no_storno'])
            # Unique Numbers Called
            calls_summary = df_calls_filtered.copy()
            calls_summary['day'] = pd.to_datetime(calls_summary['Start time'], errors='coerce').dt.day
            calls_per_period = calls_summary.groupby('day')['Called number'].nunique()
            st.subheader(f"Unique Numbers Called Over Time (Daily, excl. weekends) - {selected_date}")
            st.line_chart(calls_per_period)

        # --- Product Distribution Pie Chart (filtered by agent/date) ---
        product_sums = df_filtered[product_cols_int].sum()
        # Distribute stornos proportionally across products for pie
        if product_sums.sum() > 0:
            pie_values = (product_sums / product_sums.sum() * (product_sums.sum() - stornos)).round(0).astype(int)
        else:
    ...
    # --- AGENT COMPARISON ---
        ...
        team_stornos = df_period[(df_period['storno'] == 1) & (df_period[product_cols_int].sum(axis=1) > 0)].shape[0]
        ...
        for agent in agent_list_no_all:
            ...
            stornos = sales_agent[(sales_agent['storno'] == 1) & (sales_agent[product_cols_int].sum(axis=1) > 0)].shape[0]
            ...
            daily_stornos = sales_agent.groupby('day').apply(lambda x: ((x['storno'] == 1) & (x[product_cols_int].sum(axis=1) > 0)).sum())
            sales_no_storno_per_day = sales_per_day.sum(axis=1) - daily_stornos
            avg_product_sold_per_day = float(sales_no_storno_per_day.mean()) if hasattr(sales_no_storno_per_day, 'mean') else 0

            product_sum = sales_agent[product_cols_int].sum().sum()
            total_stornos = sales_agent[(sales_agent['storno'] == 1) & (sales_agent[product_cols_int].sum(axis=1) > 0)].shape[0]
