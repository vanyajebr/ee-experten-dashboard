...
if sales_file and calls_file:
    ...
    if page == "Dashboard":
        ...
        # --- LINE CHARTS and PIE CHART with CORRECT FILTERING (df_filtered) ---
        ...
        if selected_date == "All Time":
            ...
            # Products Sold No Storno (consistent function, uses only filtered data)
            monthly_products_sold_no_storno = df_filtered.groupby('month_period').apply(lambda x: get_products_sold_no_storno(x, product_cols_int))
    ...
    # --- AGENT COMPARISON ---
        ...
        for agent in agent_list_no_all:
            ...
            avg_call_duration = calls_3min['Duration'].mean() / 60 if not calls_3min.empty else 0
            sales_rate = (total_deals / unique_called_numbers) * 100 if unique_called_numbers else 0
