...
if sales_file and calls_file:
    ...
    if page == "Dashboard":
        ...
        # For percent of team sales
        team_data = filter_period(df_sales, 'createdate_berlin', selected_date)
        team_data = make_product_int_cols(team_data)
        team_product_cols_int = [col + '_int_only' for col in product_cols if col + '_int_only' in team_data.columns]
        team_total_products_sold = team_data[team_product_cols_int].sum().sum()
        team_total_stornos = team_data[team_data['storno'] == 1].shape[0]
        team_total_products_no_storno = team_total_products_sold - team_total_stornos

        percent_of_team_sales = (
            "100.0%" if selected_agent == "All Agents" else
            f"{(total_products_sold_no_storno / team_total_products_no_storno * 100):.1f}%" if team_total_products_no_storno else "—"
        )
        ...
        if selected_date == "All Time":
            ...
            # Products Sold (No Storno) per month = sum(products) - sum(stornos), uses same logic as dashboard metric
            monthly = []
            for m in manual_order:
                if m in df_filtered['month_period'].astype(str).values:
                    df_m = df_filtered[df_filtered['month_period'].astype(str) == m]
                    p = df_m[product_cols_int].sum().sum()
                    s = df_m[df_m['storno'] == 1].shape[0]
                    monthly.append(p - s)
                else:
                    monthly.append(0)
            monthly_products_sold_no_storno = pd.Series(monthly, index=manual_order)
            ...
            deals_per_period = []
            unique_numbers_per_period = []
            for m in manual_order:
                df_m = df_filtered[df_filtered['month_period'].astype(str) == m]
                call_m = df_calls_filtered[df_calls_filtered['month_period'].astype(str) == m]
                deals = df_m[['full_name']].drop_duplicates().shape[0]
                unique_num = call_m['Called number'].dropna().nunique()
                deals_per_period.append(deals)
                unique_numbers_per_period.append(unique_num)
            sales_rate_per_period = pd.Series([
                (d / n * 100) if n else 0
                for d, n in zip(deals_per_period, unique_numbers_per_period)
            ], index=manual_order)
            st.subheader("Sales Rate Over Time (Monthly)")
            st.line_chart(sales_rate_per_period)

            # Storno Rate Over Time (All Agents only)
            if selected_agent == "All Agents":
                storno_rate_per_period = []
                for m in manual_order:
                    df_m = df_filtered[df_filtered['month_period'].astype(str) == m]
                    products = df_m[product_cols_int].sum().sum()
                    stornos = df_m[df_m['storno'] == 1].shape[0]
                    rate = (stornos / products * 100) if products else 0
                    storno_rate_per_period.append(rate)
                storno_rate_per_period = pd.Series(storno_rate_per_period, index=manual_order)
            ...
            for m in manual_order:
                ...
                unique_num = call_m['Called number'].dropna().nunique()
                unique_numbers_per_period.append(unique_num)
            unique_numbers_per_period = pd.Series(unique_numbers_per_period, index=manual_order)
            ...
            # Products Sold (No Storno) per day = sum(products) - sum(stornos)
            if len(df_filtered) > 0:
                all_days = sorted(df_filtered['day'].unique())
            else:
                all_days = []
            daily = []
            for d in all_days:
                df_d = df_filtered[df_filtered['day'] == d]
                p = df_d[product_cols_int].sum().sum()
                s = df_d[df_d['storno'] == 1].shape[0]
                daily.append(p - s)
            daily_products_sold_no_storno = pd.Series(daily, index=all_days)
