...
if sales_file and calls_file:
    ...
    if page == "Dashboard":
        ...
        # NEW: Your business logic for "Products Sold No Storno"
        ...
        if selected_date == "All Time":
            ...
            # Products Sold (No Storno) per month: your business logic!
            ...
            # Sales Rate calculation for monthly graph (corrected)
            deals_per_period = (
                df_sales_period
            ...
            daily_products_sold = df_filtered.groupby('day')[product_cols_int].sum().sum(axis=1)
            daily_stornos = df_filtered.groupby('day').apply(lambda x: (x['storno'] == 1).sum())
            daily_products_sold_no_storno = daily_products_sold - daily_stornos
            df_daily = pd.DataFrame({'products_sold_no_storno': daily_products_sold_no_storno}).sort_index()
        ...
        # --- Product Distribution Pie Chart (using business logic: subtract stornos) ---
        ...
        # For the pie, split the subtraction across products proportionally for a better visual
    ...
    # --- AGENT COMPARISON ---
        ...
        for agent in agent_list_no_all:
            ...
            # This is business logic: sales per day - stornos / days
            stornos = sales_agent[sales_agent['storno'] == 1].shape[0]
