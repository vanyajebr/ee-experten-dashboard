...
if sales_file and calls_file:
    ...
    if page == "Dashboard":
        ...
        # Simple business logic for products sold no storno
        ...
        sales_no_storno_per_day = sales_per_day.sum(axis=1) - total_stornos / len(sales_per_day) if len(sales_per_day) > 0 else 0
        ...
        if selected_date == "All Time":
            df_sales_period = df_filtered.copy()  # <<--- use the ALREADY agent-filtered period
            df_sales_period['month_period'] = pd.to_datetime(df_sales_period['createdate_berlin'], errors='coerce').dt.to_period('M')
            df_sales_period = make_product_int_cols(df_sales_period)
            manual_order = ["2024-12", "2025-01", "2025-02", "2025-03", "2025-04"]

            # Products Sold (No Storno) per month: simple logic
            product_cols_int = [col + '_int_only' for col in product_cols if col + '_int_only' in df_sales_period.columns]
            monthly_products_sold = df_sales_period.groupby('month_period')[product_cols_int].sum().sum(axis=1)
            monthly_stornos = df_sales_period.groupby('month_period').apply(lambda x: (x['storno'] == 1).sum())
            monthly_products_sold_no_storno = monthly_products_sold - monthly_stornos
            df_monthly = pd.DataFrame({'products_sold_no_storno': monthly_products_sold_no_storno})
            df_monthly.index = df_monthly.index.astype(str)
            df_monthly = df_monthly.reindex(manual_order, fill_value=0)
            df_monthly.index = pd.CategoricalIndex(df_monthly.index, categories=manual_order, ordered=True)
            st.subheader("Products Sold (No Storno) Over Time (Monthly)")
            st.line_chart(df_monthly)

            # Sales Rate calculation for monthly graph
            deals_per_period = (
                df_sales_period
                .groupby('month_period')[['kunde_vorname', 'kunde_nachname']]
                .apply(lambda x: (x['kunde_vorname'].astype(str).str.strip() + " " + x['kunde_nachname'].astype(str).str.strip()).nunique())
            )
            calls_period = df_calls_filtered.copy()  # <<--- agent/period filtered
            calls_period['month_period'] = pd.to_datetime(calls_period['Start time'], errors='coerce').dt.to_period('M')
            unique_numbers_per_period = calls_period.groupby('month_period')['Called number'].nunique()
            ...
            total_products_per_period = df_sales_period.groupby('month_period')[product_cols_int].sum().sum(axis=1)
            storno_per_period = df_sales_period[df_sales_period['storno'] == 1].groupby('month_period').size()
            ...
            # DO NOT redefine df_filtered or df_calls_filtered!
            # Only exclude weekends for the daily breakdown
            df_filtered['createdate_berlin'] = pd.to_datetime(df_filtered['createdate_berlin'], errors='coerce')
            df_filtered['weekday'] = df_filtered['createdate_berlin'].dt.weekday
            df_filtered_no_weekends = df_filtered[~df_filtered['weekday'].isin([5, 6])]
            df_calls_filtered['Start time'] = pd.to_datetime(df_calls_filtered['Start time'], errors='coerce')
            df_calls_filtered['weekday'] = df_calls_filtered['Start time'].dt.weekday
            df_calls_filtered_no_weekends = df_calls_filtered[~df_calls_filtered['weekday'].isin([5, 6])]
            df_filtered_no_weekends['day'] = df_filtered_no_weekends['createdate_berlin'].dt.day
            df_calls_filtered_no_weekends['day'] = df_calls_filtered_no_weekends['Start time'].dt.day
            period_col = 'day'
            chart_title_suffix = f"Over Time (Daily, excl. weekends) - {selected_date}"
            # Products Sold (No Storno)
            product_cols_int = [col + '_int_only' for col in product_cols if col + '_int_only' in df_filtered_no_weekends.columns]
            daily_product_sum = df_filtered_no_weekends.groupby(period_col)[product_cols_int].sum()
            daily_stornos = df_filtered_no_weekends[df_filtered_no_weekends['storno'] == 1].groupby(period_col).size()
            daily_total_products_no_storno = daily_product_sum.sum(axis=1) - daily_stornos.reindex(daily_product_sum.index, fill_value=0)
            df_daily = pd.DataFrame({'products_sold_no_storno': daily_total_products_no_storno}).sort_index()
            st.subheader(f"Products Sold (No Storno) Over Time (Daily, excl. weekends) - {selected_date}")
            st.line_chart(df_daily)

            unique_numbers_per_period = df_calls_filtered_no_weekends.groupby('day')['Called number'].nunique()
    ...
    # --- AGENT COMPARISON ---
        ...
        for agent in agent_list_no_all:
            ...
            stornos = sales_agent[sales_agent['storno'] == 1].shape[0]
            sales_no_storno_per_day = sales_per_day.sum(axis=1) - (stornos / len(sales_per_day) if len(sales_per_day) > 0 else 0)
