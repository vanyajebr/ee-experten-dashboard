...
def get_products_sold_no_storno(df, product_cols_int):
    total_products_sold = df[product_cols_int].sum().sum()
    total_stornos = df[df['storno'] == 1].shape[0]
    return total_products_sold - total_stornos
...
if sales_file and calls_file:
    ...
    if page == "Dashboard":
        ...
        # --- Key Metrics using the SAME FUNCTION everywhere ---
        total_products_sold_no_storno = get_products_sold_no_storno(df_filtered, product_cols_int)
        ...
        # For percent of team sales, get the correct denominator for the period
        df_team_period_metrics = filter_period(df_sales, 'createdate_berlin', selected_date)
        df_team_period_metrics = make_product_int_cols(df_team_period_metrics)
        product_cols_int_team = [col + '_int_only' for col in product_cols if col + '_int_only' in df_team_period_metrics.columns]
        total_products_no_storno_team_period = get_products_sold_no_storno(df_team_period_metrics, product_cols_int_team)
        ...
        stornos_per_day = df_filtered[df_filtered['storno'] == 1].groupby('day').size()
        sales_no_storno_per_day = sales_per_day.sum(axis=1) - stornos_per_day.reindex(sales_per_day.index, fill_value=0)
        ...
        # --- LINE CHARTS ---
        if selected_date == "All Time":
            df_sales_period = filter_period(df_sales, 'createdate_berlin', selected_date)
            ...
            # Products Sold No Storno (consistent function)
            monthly_products_sold_no_storno = df_sales_period.groupby('month_period').apply(lambda x: get_products_sold_no_storno(x, product_cols_int))
            ...
            calls_period = filter_period(df_calls, 'Start time', selected_date)
            ...
            df_calls_filtered['day'] = df_calls_filtered['Start time'].dt.day
            # Daily Products Sold No Storno (consistent)
            daily_products_sold_no_storno = df_filtered.groupby('day').apply(lambda x: get_products_sold_no_storno(x, product_cols_int))
        ...
        # --- Product Distribution Pie Chart (for current filter/agent) ---
        product_sums = df_filtered[product_cols_int].sum()
        pie_units = product_sums.sum() if len(df_filtered) > 0 else 0
    ...
    # --- AGENT COMPARISON ---
        ...
        team_products_no_storno = get_products_sold_no_storno(df_period, product_cols_int)
        ...
        for agent in agent_list_no_all:
            ...
            products_no_storno = get_products_sold_no_storno(sales_agent, product_cols_int)
            percent_of_team_sales = (products_no_storno / team_products_no_storno * 100) if team_products_no_storno else 0
            ...
            stornos_per_day = sales_agent[sales_agent['storno'] == 1].groupby('day').size()
            sales_no_storno_per_day = sales_per_day.sum(axis=1) - stornos_per_day.reindex(sales_per_day.index, fill_value=0)
